...
| 5 | `assemble_output.py` | Takes all the merged, timestamped field data and fills in the data dictionary template (from F3) to produce the final `data_dictionary.md`. Substitutes the 3 header placeholders using `.replace()` in this fixed order: 1) `{table_name}` → actual table name, 2) `{source_file}` → actual source file name, 3) `{generation_date}` → actual timestamp. Substitution happens BEFORE any field data is inserted into the template. Do NOT use Python's `str.format()` — it throws `KeyError` on unrecognized curly-brace patterns in the template. Before writing field data to Markdown tables, applies `sanitize_for_markdown()` to escape special characters in field names: `\|` → `\\|`, `*` → `\*`, `` ` `` → `` \` ``, `\` → `\\`, `[` → `\[`, `]` → `\]`. This escaping applies ONLY to the Markdown output — intermediate JSON files retain raw unescaped values. Display conversion rules for Markdown table cells: `[]` (empty list) → `None`, `null`/`None` → `None`, `""` (empty string) → `None`, `true`/`True` → `true`, `false`/`False` → `false`. These conversions apply ONLY to the Markdown output — intermediate JSON files retain the original values. Checks that the template file exists before proceeding — if missing, stops with a clear error message. | **Produces the deliverable.** This is the document the user actually receives — the whole point of the skill. | Post-LLM — Step 3 |
| 6 | `generate_qa_report.py` | Counts everything: how many fields got descriptions, how many have citations, how many are flagged for clarification. After computing the confidence distribution, verifies that High + Medium + Low + N/A = total field count. If the sum doesn't match, adds a warning to the Warnings section: "Pipeline integrity warning: Confidence distribution sum ({sum}) does not match total field count ({total})." The pipeline continues and produces output — no crash. Produces `qa_report.md` with coverage stats and warnings. Checks that the template file exists before proceeding — if missing, stops with a clear error message. | **Trust but verify.** The QA report lets the user see at a glance whether the data dictionary is complete or if items need attention. | Post-LLM — Step 4 (final) |
