...
**Created**: 2026-04-06
**Status**: Draft
**Owner**: Molly Lowell, Whitney Robinson (PM), Sheila Green
...
└── tools/
    ├── validate_inputs.py            ← Validates 3 JSON input files
    ├── build_artifact_registry.py    ← Builds normalized artifact registry
    ├── map_evidence_to_controls.py   ← Maps evidence to 4 controls
...
**Biggest risk:** Phase 3 — LLM narrative quality. Mitigated by building Phases 1–2 first so we have real mapped evidence to test the SKILL.md against, rather than designing the prompt in a vacuum.
