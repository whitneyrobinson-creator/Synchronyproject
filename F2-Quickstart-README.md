...
**Note:** This step needs the F3 template. If F3 hasn't delivered the template yet, this script won't work. Steps 1–6 work without F3.
...
**Note:** Same as Step 7 — needs the F3 template for `qa_report_template.md`.
...
|---------|-----------------|
| Pipeline Metadata | Correct table name and field count? |
| Coverage Summary | What percentage of fields got real descriptions? |
| Flagged Fields | Which fields need human review? |
| Merge Corrections | Did F2 auto-fix anything? Does the fix make sense? |
| Extraction Warnings | Any duplicate field names? |
| Run Summary | Clean run, partial, or failed? |
...
| `assemble_output.py` fails | F3 template is missing | Check that `assets/data_dictionary_template.md` exists. Steps 1–6 work without it. |
...
Steps 7 and 8 (`assemble_output.py` and `generate_qa_report.py`) need the F3 templates to produce the final Markdown files. Once F3 delivers those templates, you can test the full pipeline end to end.
